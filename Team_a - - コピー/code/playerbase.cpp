...
void CPlayerBase::PlayerCollision()
{
	...
	// モーションの動き
	D3DXMATRIX mtxWorld = GetModel().NumModel[nCnt].mtxWorld;
	...
	if (MotionInfo[m_MotionType].nAtkStar == MotionInfo[m_MotionType].nNumKey)
	{// 当たり判定の可視化
		...
		m_Hitmodel = CHitModel::Create(pPlayer[1], pos, D3DXVECTOR3(m_fAttack, m_fAttack, m_fAttack));
	}
	...
	{
		float fRadius = pPlayer[1]->m_fRadius + m_fAttack;
