...
#define POD_ROT(e)				(D3DXToRadian(180 / 4 * e))
...
void CPlayerBase::PlayerPad(int cnt)
{
	...
	// 入力された X、Y
	float fH, fV;
	pGamePad->GetJoypadStickLeft(cnt, &fH, &fV);
	...
	if (m_PlayerState != PLAYERSTATE_ATK && m_MotionType != MOTIONTYPE_DAMAGE && m_MotionType != MOTIONTYPE_STANDUP)
	{
		...
		if (bJump == false && m_MotionType != MOTIONTYPE_RAND)
		{
			if (CManager::GetMode() == CManager::MODE_GAME)
			{
				...
				else if (pKeyboard->GetKeyboardPress(DIK_A) || pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_LEFT) || (-POD_ROT(1) > fRot && fRot > -POD_ROT(3)))
				{//  A キー操作
				...
				else if (pKeyboard->GetKeyboardPress(DIK_D) || pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_RIGHT) || (POD_ROT(1) < fRot && fRot < POD_ROT(3)))
				{//  D キー操作
					m_move.x += D3DX_PI*0.5f * m_fSpeed;
					m_fDiffrot.y = D3DX_PI*-0.5f;
					MotionChangePlayer(MOTIONTYPE_RUN);
				...
				else if (pKeyboard->GetKeyboardPress(DIK_W) || pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_UP) || (-POD_ROT(1) < fRot && fRot < POD_ROT(1)))
				{// W キー操作
					if (pKeyboard->GetKeyboardPress(DIK_SPACE) || pGamePad->GetJoypadTrigger(cnt, CPad::JOYPADKEY_B))
					{
						MotionChangePlayer(MOTIONTYPE_UPATK);
				...
				else if (pKeyboard->GetKeyboardPress(DIK_S) || pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_DOWN) || (-POD_ROT(3) >= fRot || fRot >= POD_ROT(3)))
				{
					MotionChangePlayer(MOTIONTYPE_CROUCHWAIT);
					if (pKeyboard->GetKeyboardPress(DIK_SPACE) || pGamePad->GetJoypadTrigger(cnt, CPad::JOYPADKEY_B))
					{
		...
		{
			if (m_PlayerState != PLAYERSTATE_AIRATK && m_MotionType != MOTIONTYPE_DAMAGE && m_MotionType != MOTIONTYPE_DOWN)
			{
				...
				if (pKeyboard->GetKeyboardPress(DIK_A) || pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_LEFT) || (-POD_ROT(1) > fRot && fRot > -POD_ROT(3)))
				{//  A キー操作
	...
	case PLAYERTYPE::PLAYERTYPE_KEN:
		...
		case MOTIONTYPE_SP_N:
			...
			if (nCountATK == 20)
			{
				if (m_fDiffrot.y == D3DX_PI*0.5f)
				{
					if (cnt == 0)
					{
						CBullet::Create(GetPos(), D3DXVECTOR3(-10, 0.0f, 0.0f), CBullet::BULLETTEX_KEN, CMaker::MAKERTYPE_1P, 40);
					}
					if (cnt == 1)
					{
						CBullet::Create(GetPos(), D3DXVECTOR3(-10, 0.0f, 0.0f), CBullet::BULLETTEX_KEN, CMaker::MAKERTYPE_2P, 40);
					}
				}
				else
				{
		...
		case MOTIONTYPE_AIR_U:
		case MOTIONTYPE_AIR_D:
			if (pKeyboard->GetKeyboardPress(DIK_D) || pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_RIGHT) || (POD_ROT(1) < fRot && fRot < POD_ROT(3)))
			{
			...
			if (pKeyboard->GetKeyboardPress(DIK_S) || pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_DOWN) || (-POD_ROT(1) < fRot && fRot < POD_ROT(1)))
			{
				m_move.y -= 1.2f;
			}
			break;
	...
	case PLAYERTYPE::PLAYERTYPE_SWORD:
		...
		case MOTIONTYPE_AIR_U:
		case MOTIONTYPE_AIR_D:
			if (pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_RIGHT) || (POD_ROT(1) < fRot && fRot < POD_ROT(3)))
			{
				m_move.x += D3DX_PI*0.5f* m_fAirAtkSpeed;
			}
			else if (pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_LEFT) || (-POD_ROT(1) > fRot && fRot > -POD_ROT(3)))
			{
				m_move.x += D3DX_PI*-0.5f* m_fAirAtkSpeed;
			}
			if (pGamePad->GetJoypadPress(cnt, CPad::JOYPADKEY_DOWN) || (-POD_ROT(1) < fRot && fRot < POD_ROT(1)))
			{
				m_move.y -= 1.2f;
			}
			break;
